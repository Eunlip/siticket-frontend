...
const MyComplaint: React.FC = () => {
	...
	const checkIsCompleted = (id: number) => {
		const ok = window.confirm('Are you sure you want to mark this complaint as completed?');
	...
	return (
		<div className='sm:container dark:bg-boxdark mx-auto overflow-x-scroll sm:overflow-hidden shadow bg-neutral-50 shadow-neutral-300 dark:shadow-neutral-700 rounded-md px-5 py-10'>
			<div className='mb-10 flex flex-col sm:flex-row sm:justify-between gap-5 sm:gap-2'>
				...
				<Link
					to='/add-complaint'
					className='px-3 mx-auto sm:mx-0 py-2 border rounded-md bg-blue-600 text-white dark:border-blue-500 text-sm flex items-center gap-1 hover:bg-blue-600/90 transition-all font-medium'
				>
			...
			<div className='overflow-x-scroll sm:overflow-hidden'>
				<DataTable
					...
					className='display table overflow dark:border-graydark border border-neutral-200 shadow-sm w-full'
					slots={{
						6: (data: any, row: any) => (
							<div className='flex items-center space-x-3.5'>
								<button
									...
									className={`text-blue-600 ${
										row.keterangan === 'closed' ? 'hidden' : 'hover:text-blue-400'
									}`}
								...
								<a
									...
									className={`text-yellow-500 ${
										row.keterangan === 'closed' ? 'hidden' : 'hover:text-yellow-400'
									}`}
								>
