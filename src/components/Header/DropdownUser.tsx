...
const DropdownUser = () => {
	...
	const getCurrentUser = async () => {
		try {
			...
			if (role === 'tc') {
				const response = await axiosInstance.get('/api/tc/user/me', {
					headers: {
						Authorization: `Bearer ${token}`,
					},
				});
		...
		<ClickOutside onClick={() => setDropdownOpen(false)} className='relative'>
			...
			>
				{currentUser?.map((user) => (
					<div key={user.id} className='block text-right'>
						<span className='block text-sm font-medium text-black dark:text-white'>
							{user.name}
						</span>
						<span className='block text-xs capitalize'>
							({user.role === 'admin' || user.role === 'tc' ? user.role : 'guest'})
						</span>
					</div>
				))}
			...
			{dropdownOpen && (
				...
				>
					<ul className='flex flex-col gap-5 border-b border-stroke px-6 py-4 dark:border-strokedark'>
						<li className='flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out cursor-default lg:text-base capitalize'>
							<MdOutlineAdminPanelSettings className='text-neutral-500 text-xl dark:text-bodydark' />
							{role === 'admin' || role === 'tc' ? role : 'guest'}
						</li>
					</ul>
					<button
